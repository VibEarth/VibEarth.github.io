...
위에서 업로드 한 코드는 AP 모드로 설정하여 ESP32가 직접 핫스팟을 만듭니다.
스마트폰에서 WiFi 목록 중 `C3-Weather` (password : 12345678 )에 접속합니다. 
와이파이로 ESP32에 연결하기 때문에 인터넷 접속은 잠시 중단됩니다.

## ESP32 웹서버 페이지 접속

스마트폰에서 브라우저앱을 열어 http://192.168.4.1 에 접속하면 이런 화면이 나옵니다.

<!-- 스크린샷 추가 예정 -->

데이터 카드 4개(온도, 습도, 기압, 고도)와 그래프 2개(온도/습도, 기압/고도)가 표시됩니다.
