...
??? note "지극히 개인적인 웹서버 스토리"
	...
	장고는 파이썬으로 홈페이지를 만들 수 있는 프레임워크(도구, 골격)였습니다!
	그 뒤 개인 블로그 홈페이지를 만들기 위해서 파이썬, 장고 관련 서적과 IT 블로그들을 몇 달간 뒤졌습니다.  
	홈페이지를 만들려면 서버와 도메인이 있어야 하고, 웹페이지를 만드는 기본골격(프레임워크)도 필요했습니다.
## ESP32를 공유기처럼
