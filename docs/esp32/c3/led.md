...
> 모든 프로그래밍 입문이 "Hello, World!"로 시작하듯, 하드웨어 입문은 LED 블링킹으로 시작합니다.

ESP32-C3 SuperMini에는 GPIO8에 내장 LED가 달려 있습니다. 코드 한 줄로 이 LED를 껐다 켰다 할 수 있다는 게 별것 아닌 것 같지만, 처음으로 "내가 짠 코드가 물리적인 무언가를 움직였다"는 감각은 생각보다 꽤 강렬합니다.
...
UNO 같은 공식 보드에서는 이 매크로가 정확한 핀 번호로 자동 연결되지만, SuperMini는 서드파티 보드라 보드 정의에 따라 `LED_BUILTIN`이 엉뚱한 핀을 가리킬 수 있습니다.
...
> **SuperMini에서는 `LED_BUILTIN` 대신 핀 번호 `8`을 직접 명시하는 것이 안전합니다.**

### delay 값을 바꿔보세요

`delay(1000)`의 숫자는 밀리초(ms) 단위입니다. 1000 = 1초.
...
LED 하나 깜빡이는 게 전부지만, 여기서 익힌 구조 — `setup()`에서 핀 설정, `loop()`에서 반복 제어, 버튼 입력 읽기 — 는 앞으로 BME280 센서를 읽고 측정값이 임계치를 넘으면 신호를 보내는 코드까지 그대로 이어집니다.

다음은 기상관측기의 첫 번째 센서, BME280으로 넘어갑니다.