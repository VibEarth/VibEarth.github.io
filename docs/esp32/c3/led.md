...
> ESP32는 기본적으로 아두이노 IDE의 C++ 언어로 프로그래밍 되어 작동합니다.  
> 물론, C++ 코딩이 처음이어도 괜찮습니다. 우리에게는 AI가 있으니까요.

ESP32-C3 SuperMini에는 전원이 연결되면 켜지는 빨간LED와 GPIO8에 연결된 상태표시용 파랑LED가 내장되어 있습니다. 코드 몇 줄로 LED를 껐다 켰다 할 수 있다는 게 별것 아닌 것 같지만, 처음으로 "내가 짠 코드가 물리적인 무언가를 움직였다"는 감각은 생각보다 꽤 강렬합니다. 그 강렬함으로 여러분을 초대합니다🜨
...
아두이노 IDE에서 C3로 업로드하면 1초 간격으로 파랑 LED가 깜빡입니다.
...
> 보드매니저에서 `MakerGO ESP32 C3 SuperMini`를 선택하면 이 기능도 작동을 하지만, `ESP32C3 Dev Module` 선택 시 핀설정이 달라 작동하지 않습니다.
